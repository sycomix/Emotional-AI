...
'''
def batch_creator(batch_size, X_train, y_train,idx,dim_input,n_classes):
    """Create batch and return appropriate format"""
    num=int(X_train.shape[0]/batch_size)
    dataset_length=X_train.shape[0]
    #batch_mask = rng.choice(dataset_length, batch_size)
    batch_mask = np.arange(idx,dataset_length, num)
    batch_x = X_train[[batch_mask]].reshape(-1, dim_input)
    #batch_x = preproc(batch_x)
    batch_y = y_train[[batch_mask]]
    batch_y = dense_to_one_hot(batch_y,n_classes)
